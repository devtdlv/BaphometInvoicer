            ...
            <div class="form-group">
                ...
                <select name="currency_code" class="form-input" id="currency_select" required>
                    @foreach($currencies as $currency)
                        <option value="{{ $currency['code'] }}" data-symbol="{{ $currency['symbol'] }}" {{ $quote->currency_code === $currency['code'] ? 'selected' : '' }}>
                            {{ $currency['label'] }}
                        </option>
                    @endforeach
                </select>
                <input type="hidden" name="currency_symbol" id="currency_symbol" value="{{ $quote->currency_symbol }}">
            </div>

            <div class="form-group">
                <label class="form-label">Exchange Rate</label>
                <input type="number" name="currency_rate" class="form-input" value="{{ old('currency_rate', $quote->currency_rate) }}" step="0.000001" min="0.000001">
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="pdf_template" class="form-input">
                @foreach($pdfTemplates as $key => $label)
                    <option value="{{ $key }}" {{ $quote->pdf_template === $key ? 'selected' : '' }}>{{ $label }}</option>
                @endforeach
            </select>
        </div>
    </div>
