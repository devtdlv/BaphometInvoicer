@extends('layouts.app')

@section('title', 'Quote ' . $quote->quote_number)

@section('content')
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
    <h1 style="font-size: 2rem; font-weight: 700;">Quote {{ $quote->quote_number }}</h1>
    <div style="display: flex; gap: 1rem;">
        @if($quote->status === 'draft')
            <form method="POST" action="{{ route('quotes.send', $quote) }}" style="display: inline;">
                @csrf
                <button type="submit" class="btn btn-primary">Send Quote</button>
            </form>
        @endif
        @if($quote->status === 'sent')
            <form method="POST" action="{{ route('quotes.convert', $quote) }}" style="display: inline;">
                @csrf
                <button type="submit" class="btn btn-primary">Convert to Invoice</button>
            </form>
        @endif
        <a href="{{ route('quotes.pdf', $quote) }}" class="btn btn-secondary">Download PDF</a>
        <a href="{{ route('quotes.edit', $quote) }}" class="btn btn-secondary">Edit</a>
    </div>
</div>

<div class="card">
    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 2rem; margin-bottom: 2rem;">
        <div>
            <h3 style="margin-bottom: 0.5rem; font-weight: 600;">From</h3>
            <p style="color: var(--text-secondary);">{{ auth()->user()->name }}</p>
            <p style="color: var(--text-secondary);">{{ auth()->user()->email }}</p>
        </div>
        <div>
            <h3 style="margin-bottom: 0.5rem; font-weight: 600;">To</h3>
            <p style="color: var(--text-secondary);">{{ $quote->client->name }}</p>
            <p style="color: var(--text-secondary);">{{ $quote->client->email }}</p>
            @if($quote->client->company)
                <p style="color: var(--text-secondary);">{{ $quote->client->company }}</p>
            @endif
        </div>
    </div>
    
    <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; margin-bottom: 2rem; padding: 1rem; background: var(--bg-tertiary); border-radius: 0.5rem;">
        <div>
            <p style="font-size: 0.875rem; color: var(--text-secondary); margin-bottom: 0.25rem;">Status</p>
            <p style="font-weight: 600;">
                @if($quote->status === 'accepted')
                    <span class="badge badge-success">Accepted</span>
                @elseif($quote->status === 'sent')
                    <span class="badge badge-warning">Sent</span>
                @elseif($quote->status === 'rejected')
                    <span class="badge badge-danger">Rejected</span>
                @elseif($quote->status === 'expired')
                    <span class="badge badge-danger">Expired</span>
                @else
                    <span class="badge badge-info">Draft</span>
                @endif
            </p>
        </div>
        <div>
            <p style="font-size: 0.875rem; color: var(--text-secondary); margin-bottom: 0.25rem;">Issue Date</p>
            <p style="font-weight: 600;">{{ $quote->issue_date->format('M d, Y') }}</p>
        </div>
        <div>
            <p style="font-size: 0.875rem; color: var(--text-secondary); margin-bottom: 0.25rem;">Expiry Date</p>
            <p style="font-weight: 600;">{{ $quote->expiry_date ? $quote->expiry_date->format('M d, Y') : 'N/A' }}</p>
        </div>
        <div>
            <p style="font-size: 0.875rem; color: var(--text-secondary); margin-bottom: 0.25rem;">Total</p>
            <p style="font-weight: 600; font-size: 1.25rem; color: var(--accent);">
                {{ $quote->currency_symbol }}{{ number_format($quote->total, 2) }}
                <span style="font-size: 0.85rem; color: var(--text-secondary);">{{ $quote->currency_code }}</span>
            </p>
        </div>
    </div>
    
    <table class="table">
        <thead>
            <tr>
                <th>Description</th>
                <th>Quantity</th>
                <th>Price</th>
                <th>Total</th>
            </tr>
        </thead>
        <tbody>
            @foreach($quote->items as $item)
                <tr>
                    <td>{{ $item->description }}</td>
                    <td>{{ $item->quantity }}</td>
                    <td>{{ $quote->currency_symbol }}{{ number_format($item->price, 2) }}</td>
                    <td>{{ $quote->currency_symbol }}{{ number_format($item->total, 2) }}</td>
                </tr>
            @endforeach
        </tbody>
        <tfoot style="border-top: 2px solid var(--border);">
            <tr>
                <td colspan="3" style="text-align: right; font-weight: 600;">Subtotal:</td>
                <td style="font-weight: 600;">{{ $quote->currency_symbol }}{{ number_format($quote->subtotal, 2) }}</td>
            </tr>
            @if($quote->discount_amount > 0)
                <tr>
                    <td colspan="3" style="text-align: right; font-weight: 600;">Discount:</td>
                    <td style="font-weight: 600; color: var(--success);">-{{ $quote->currency_symbol }}{{ number_format($quote->discount_amount, 2) }}</td>
                </tr>
            @endif
            @if($quote->tax_amount > 0)
                <tr>
                    <td colspan="3" style="text-align: right; font-weight: 600;">Tax ({{ $quote->tax_rate }}%):</td>
                    <td style="font-weight: 600;">{{ $quote->currency_symbol }}{{ number_format($quote->tax_amount, 2) }}</td>
                </tr>
            @endif
            <tr>
                <td colspan="3" style="text-align: right; font-weight: 700; font-size: 1.125rem;">Total:</td>
                <td style="font-weight: 700; font-size: 1.125rem; color: var(--accent);">
                    {{ $quote->currency_symbol }}{{ number_format($quote->total, 2) }} {{ $quote->currency_code }}
                </td>
            </tr>
        </tfoot>
    </table>
    
    @if($quote->notes)
        <div style="margin-top: 2rem;">
            <h3 style="margin-bottom: 0.5rem; font-weight: 600;">Notes</h3>
            <p style="color: var(--text-secondary);">{{ $quote->notes }}</p>
        </div>
    @endif
    
    @if($quote->terms)
        <div style="margin-top: 1.5rem;">
            <h3 style="margin-bottom: 0.5rem; font-weight: 600;">Terms</h3>
            <p style="color: var(--text-secondary);">{{ $quote->terms }}</p>
        </div>
    @endif
</div>
@endsection

